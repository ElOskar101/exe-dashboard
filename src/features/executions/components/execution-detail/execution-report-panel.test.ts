...
describe('execution-report-panel helpers', () => {
  it('builds an absolute trailing-slash report base URL', () => {
    expect(resolveExecutionReportBaseUrl('/execution-reports/exe-1', 'https://dashboard.test')).toBe(
      'https://dashboard.test/execution-reports/exe-1/',
    )
  })

  it('injects a base tag and URL shim into a report head', () => {
    const reportBaseUrl = 'https://dashboard.test/execution-reports/exe-1/'
  ...
  it('prepends the runtime shim when the report has no head tag', () => {
    const html = isolateReportHtml('<body>Report</body>', 'https://dashboard.test/execution-reports/exe-1/')

    expect(html.trimStart().startsWith('<base href="https://dashboard.test/execution-reports/exe-1/" />')).toBe(true)
  })
})
