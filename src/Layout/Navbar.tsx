...
import TablerIcons, { availableIconsTypes } from "../components/TablerIcons";
...
export default function Navbar() {
    ...
    useEffect(() => {
        setRoutes(() => t('routes', { returnObjects: true }) as Array<{
            icon: availableIconsTypes,
            route: string,
            label: string,
        }>)
    ...
    return (
        <nav className="w-full text-[var(--neutral-900)] shadow-sm dark:bg-[var(--primary-50)] dark:text-[var(--neutral-900)]">
          ...
          `}
                >
                    {routes.map(route =>
                        <a className="text-[var(--neutral-900)] hover:text-[var(--accent-600)] dark:hover:text-[var(--accent-500)] transition-colors cursor-pointer flex items-center gap-x-1" href={route.route}>
